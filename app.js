...
const viewSearch = $(".nav__searchMp3"); 
...
const app = {
    ...
    searchMp3Api: function (dataSearch) {
        const list  = fetch(dataSearch)
            ...
            .then(items =>{
                this.renderSongSearches()
            })
    },

    renderSongSearches: function () {
        // viewSearch
        let abc = this.songList.map(item =>{
            console.log(item);
            return `
            <li><h3>${item.name}</h3>  <span>${item.artist}</span></li>
            <hr>
            `
        })
        // console.log(abc.join(''));
        viewSearch.innerHTML = abc.join('');
    },
